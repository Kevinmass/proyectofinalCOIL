...
public:
    ...
    int GetDepositos()
    {
        ...
        Depositos = (DatosL.getTamanio(0)) - 3;
    ...
    void total_art_dif()
    {
        ...
        std::cout << "La cantidad de articulos diferentes es ---> " << Cantidad - 1;
    ...
    void Datos_xgrupos()
    {
        ...
        for (int i = 1; i < GetCantidad(); i++)
        {
            ...
            // Se guarda el string del grupo
            std::string DatosAUX = DatosL(i, 0);
        ...
        for (int i = pos; i < n + pos; i++)
        {
            ...
            std::cout << "[ " << contador << " ]"
                      << " "
                      << "Nombre del Articulo: " << DatosL(i, 2) << std::endl;
            contador++;
            std::cout << "            Codigo de barra: " << DatosL(i, 1) << std::endl;
            std::cout << std::endl;
            std::cout << "       --Cantidad por deposito--" << std::endl;

            // Se recorre el vector de vectores de string partiendo desde la columna 3 (primer deposito) hasta el numero de depositos
            for (int j = 3; j < DatosL.getTamanio(i); j++)
            {

                // Se guarda el string de la cantidad
                std::string cantidadStr = DatosL(i, j);
                ...
                {
                    std::cout << "       " << DatosL(0, j) << ": " << DatosL(i, j) << " ---> Estado:  normal" << std::endl;
                }

                else if (aux <= getMinStockD())
                {
                    std::cout << "       " << DatosL(0, j) << ": " << DatosL(i, j) << " ---> Estado:  se debe reponer" << std::endl;
                }

                else if (aux >= getMaxStockD())
                {
                    std::cout << "       " << DatosL(0, j) << ": " << DatosL(i, j) << " ---> Estado:  Maximo alcanzado/excediendo" << std::endl;
                }
            }
            std::cout << std::endl;
    ...
    void menu_interno()
    {
        ...
        case 8:
            ...
            std::cout << "Nota: esta funcion utiliza ListaV y puede fallar al mostrar los datos" << std::endl;
            std::cout << "si ese fuera el caso, reincie el programa y vuelva a marcar la opcion 8" << std::endl;
